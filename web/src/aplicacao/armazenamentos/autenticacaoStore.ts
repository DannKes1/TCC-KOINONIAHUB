...
// O token JWT vive apenas no cookie httpOnly (kh_token), inacessível ao JavaScript.
// Este store guarda somente dados de sessão usados pela interface (perfil, ids, validade).
...
export const usarAutenticacaoStore = defineStore("autenticacao", {
  ...
  actions: {
    ...
    entrar(dados: any) {
      // O token NÃO é mais lido nem armazenado no cliente: ele está no cookie httpOnly.
      // Guardamos apenas os metadados de sessão para a interface.
    ...
    // Encerra a sessão: pede ao servidor para apagar o cookie httpOnly e limpa o estado local.
    async sair() {
      // O estado local é limpo primeiro: sair da interface não pode depender
      // da resposta da rede. Sem isso, o clique em Sair navegava para /login
      // com a sessão ainda marcada como ativa e o guard devolvia para o painel.
      this.$reset();
      ...
      try {
        ...
        // ignora falha de rede no logout: o estado local já foi limpo
      }
    },
  },
});
